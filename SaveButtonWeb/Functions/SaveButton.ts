...
(() => {
    Office.initialize = function (reason: Office.InitializationReason) {
...
class State {
    constructor(public file: any, public counter: number, public sliceCount: number) { }
};

// Get all of the content from a PowerPoint or Word document in 100-KB chunks of text.
function writeText(event: any) {
    Office.context.document.getFileAsync(Office.FileType.Compressed,
        ...
        function (result) {
            if (result.status === Office.AsyncResultStatus.Succeeded) {
                ...
                let state = new State(myFile, 0, myFile.sliceCount);
...
function getSlice(state: State) {
    state.file.getSliceAsync(state.counter, function (result: Office.AsyncResult) {
        if (result.status === Office.AsyncResultStatus.Succeeded) {
...
function sendSlice(slice: any, state: State) {
    var data = slice.data;
